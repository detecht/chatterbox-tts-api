...
class S3Token2Mel(torch.nn.Module):
    ...
    def embed_ref(
        ...
        if isinstance(ref_wav, np.ndarray):
            ref_wav = torch.from_numpy(ref_wav).to(device=device, dtype=self.dtype)

        if ref_wav.device != device:
            ref_wav = ref_wav.to(device)

        if ref_wav.dtype != self.dtype:
            ref_wav = ref_wav.to(self.dtype)
        ...
        # Ensure correct dtype after resampling
        ...
        # Tokenize 16khz reference - Tokenizer might still need float32 for STFT
        tokenizer_input = ref_wav_16.to(torch.float32) if hasattr(self.tokenizer, 'log_mel_spectrogram') else ref_wav_16
        ref_speech_tokens, ref_speech_token_lens = self.tokenizer(tokenizer_input)
